...
in
{
  ...
  nodes.machine =
    { pkgs, ... }:
    {
      virtualisation.emptyDiskImages = [
        {
          size = 4096;
          driveConfig.deviceExtraOpts.serial = "btrfs-aux";
        }
      ];

      virtualisation.fileSystems."/mnt/btrfs" = {
        device = "/dev/disk/by-id/virtio-btrfs-aux";
        fsType = "btrfs";
        autoFormat = true;
      };
  ...
  testScript = ''
    ...
    with subtest("btrfs-presnapshot: recursive snapshot with nested subvolumes"):
        machine.succeed("btrfs subvolume create /mnt/btrfs/data")
        machine.succeed("echo 'parent data' > /mnt/btrfs/data/parentfile")
        machine.succeed("btrfs subvolume create /mnt/btrfs/data/nested")
        machine.succeed("echo 'nested data' > /mnt/btrfs/data/nested/childfile")
        machine.succeed("mkdir -p /var/lib/kopia-repo-btrfs")
        machine.succeed("systemctl start kopia-repository-btrfs-presnapshot.service")
        machine.succeed("systemctl start kopia-snapshot-btrfs-presnapshot.service")
        # Verify snapshot was created
        machine.succeed(
            "${kopiaEnv "btrfs-presnapshot"}"
            " kopia snapshot list /mnt/btrfs/data --json | jq -e 'length == 1'"
        )
        # Verify temporary btrfs snapshot was cleaned up
        machine.fail("btrfs subvolume show /mnt/btrfs/.kopia-snapshot-btrfs-presnapshot")
